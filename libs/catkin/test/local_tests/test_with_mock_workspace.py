...
class MockTest(AbstractCatkinWorkspaceTest):
    ...
    def test_catkin_only(self):
        ...
        assert_exists(self.installdir, "env.sh", "setup.sh", "setup.zsh", "setup.fish")
    ...
    def test_nolang(self):
        ...
        self.cmake(
            CATKIN_WHITELIST_PACKAGES="nolangs",
    ...
    def test_noproject(self):
        ...
        out = self.cmake(CMAKE_PREFIX_PATH=self.installdir, expect=fail)
        print("failed as expected, out={}".format(out))

        self.assertTrue(
            b"catkin_package() PROJECT_NAME is set to 'Project'" in out, out
        )
