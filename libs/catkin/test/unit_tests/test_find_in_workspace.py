...
class FindInWorkspaceTest(unittest.TestCase):
    def test_get_valid_search_dirs(self):
        self.assertEqual(
            ["bin", "etc", "include", "lib", "share"], _get_valid_search_dirs([], None)
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ["etc", "include", "libexec", "share"], _get_valid_search_dirs(None, "foo")
        )
        self.assertEqual(
            ["bin", "etc", "include", "lib", "share"],
            _get_valid_search_dirs(None, None),
        ...
        self.assertEqual(
            ["include", "bin"], _get_valid_search_dirs(["include", "bin"], None)
        )
        self.assertEqual(
            ["include", "etc"], _get_valid_search_dirs(["include", "etc"], "foo")
        )
    ...
    def test_find_in_workspaces(self):
        ...
        existing = find_in_workspaces(
            ["include"], "foo", "foopath", _workspaces=["bar"], considered_paths=checked
        )
        self.assertEqual([], existing)
        ...
        self.assertEqual([], existing)
        self.assertEqual(
            ["bar/include/foo/foopath", "baz/include/foo/foopath"], checked
        )
        checked = []
        existing = find_in_workspaces(
            ["include", "etc", "libexec"],
        ...
        checked = []
        existing = find_in_workspaces(
            None, None, None, _workspaces=["bar"], considered_paths=checked
        )
        self.assertEqual([], existing)
        self.assertEqual(
            ["bar/bin", "bar/etc", "bar/include", "bar/lib", "bar/share"], checked
        )
    ...
    def test_with_sourcepath(self):
        ...
        try:
            ...
            self.assertEqual([os.path.join(root_dir, "ws1", "include")], existing)
