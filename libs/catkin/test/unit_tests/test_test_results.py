...
class TestResultsTest(unittest.TestCase):
    def test_read_junit(self):
        try:
            ...
            with open(result_file, "w") as fhand:
                fhand.write(
                    '<testsuites tests="5" failures="3" errors="1" disabled="2" time="35" name="AllTests"></testsuites>'
                )
            (num_tests, num_errors, num_failures) = catkin_test_results.read_junit(
                result_file
            )
            self.assertEqual((5, 1, 3), (num_tests, num_errors, num_failures))
            (num_tests, num_errors, num_failures, num_skipped) = (
                catkin_test_results.read_junit2(result_file)
            )
            self.assertEqual(
                (5, 1, 3, 2), (num_tests, num_errors, num_failures, num_skipped)
            )
        finally:
    ...
    def test_read_junit_skip(self):
        try:
            ...
            with open(result_file, "w") as fhand:
                fhand.write(
                    '<testsuites tests="5" failures="3" errors="1" skip="2" time="35" name="AllTests"></testsuites>'
                )
            (num_tests, num_errors, num_failures) = catkin_test_results.read_junit(
                result_file
            )
    ...
    def test_read_junit_skipped(self):
        try:
            ...
            with open(result_file, "w") as fhand:
                fhand.write(
                    '<testsuites tests="5" failures="3" errors="1" skipped="2" time="35" name="AllTests"></testsuites>'
                )
            (num_tests, num_errors, num_failures) = catkin_test_results.read_junit(
                result_file
            )
    ...
    def test_test_results(self):
        try:
            ...
            self.assertEqual(
                {"test1.xml": (5, 1, 3, 2), "test2.xml": (5, 1, 3, 2)}, results
            )
        finally:
            shutil.rmtree(rootdir)
    ...
    def test_print_summary(self):
        ...
        try:
            ...
            summary = sys.stdout.getvalue()
            self.assertTrue(
                "5 tests, 1 errors, 3 failures, 2 skipped" in summary, summary
            )
            self.assertTrue(
                "7 tests, 2 errors, 4 failures, 1 skipped" in summary, summary
            )
            self.assertTrue(
                "12 tests, 3 errors, 7 failures, 3 skipped" in summary, summary
            )
