...
class InitWorkspaceTest(unittest.TestCase):
    def test_symlink_or_copy(self):
        try:
            ...
            _symlink_or_copy(
                join(root_dir, "subdir", "foo"), join(root_dir, "subdir", "foolink")
            )
            _symlink_or_copy(
                os.path.relpath(join(root_dir, "subdir", "foo"), os.getcwd()),
                join(root_dir, "foolinkrel"),
            )

            self.assertEqual(
                join(root_dir, "subdir", "foo"), os.readlink(join(root_dir, "foolink"))
            )
            self.assertEqual(
                join(root_dir, "subdir", "foo"),
    ...
    def test_init_workspace(self):
        try:
            ...
            with open(
                join(root_dir, "ws1", "catkin", "cmake", "toplevel.cmake"), "ab"
            ) as fhand:
            ...
            self.assertTrue(
                os.path.samefile(
                    join(
                        os.path.dirname(__file__), "..", "..", "cmake", "toplevel.cmake"
                    ),
