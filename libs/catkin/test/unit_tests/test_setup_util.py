...
data = configure_file(
    os.path.join(
        os.path.dirname(__file__), "..", "..", "cmake", "templates", "_setup_util.py.in"
    ),
    {
...
class SetupUtilTest(unittest.TestCase):
    def test_get_reversed_workspaces(self):
        try:
            ...
            mock_env = {
                "CMAKE_PREFIX_PATH": os.pathsep.join([nows, foows, barws, "invalid"])
            }
    ...
    def test_prefix_env(self):
        try:
            ...
            mock_env = {}
            self.assertEqual("", _prefix_env_variable(mock_env, "varname", [], ""))
            self.assertEqual(
                os.pathsep.join([foo_path, bar_path]),
                _prefix_env_variable(
                    mock_env, "varname", [foo_path, bar_path, baz_path], ""
                ),
            )
            ...
            self.assertEqual(
                os.pathsep.join([foo_path, lim_path]) + os.pathsep,
                _prefix_env_variable(
                    mock_env, "varname", [foo_path, lim_path, foo_path, lim_path], ""
                ),
            )
        finally:
            shutil.rmtree(rootdir)
    ...
    def test_remove_from_env(self):
        ...
        try:
            ...
            self.assertEqual(
                foolib, _rollback_env_variable(mock_env, varname, ["/lib"])
            )
            ...
            self.assertEqual(
                None, _rollback_env_variable(mock_env, varname, ["/nolib"])
            )
            ...
            self.assertEqual(
                os.pathsep.join([nowslib, foolib]),
                _rollback_env_variable(mock_env, "varname", ["/lib"]),
            )
            self.assertEqual(
                os.pathsep.join([nowslib, foolib]),
                _rollback_env_variable(mock_env, "varname", ["lib"]),
