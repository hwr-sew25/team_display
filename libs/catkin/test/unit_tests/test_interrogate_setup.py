...
imp.load_source(
    ...
    os.path.join(
        os.path.dirname(__file__), "..", "..", "cmake", "interrogate_setup_dot_py.py"
    ),
)
...
class InterrogateSetupTest(unittest.TestCase):
    def test_generate_cmake_file(self):
        cmake_lines = generate_cmake_file(
            package_name="pack1",
            version="0.0.1",
            scripts=[],
            package_dir={"": "foopath"},
        ...
        self.assertEqual(
            [
                'set(pack1_SETUP_PY_SETUP_MODULE "setuptools")',
                'set(pack1_SETUP_PY_VERSION "0.0.1")',
                'set(pack1_SETUP_PY_SCRIPTS "")',
                'set(pack1_SETUP_PY_PACKAGES "foo;bar")',
                'set(pack1_SETUP_PY_PACKAGE_DIRS "foo;bar")',
    ...
    def test_get_locations(self):
        ...
        self.assertEqual({"foo": "src/foo"}, _get_locations(["foo"], {"": "src"}))
        self.assertEqual(
            {"foo": "src/foo", "foo.bar": "src/foo/bar"},
            _get_locations(["foo.bar"], {"": "src"}),
        )
        self.assertEqual({"foo": "src"}, _get_locations(["foo"], {"foo": "src"}))
        self.assertEqual(
    ...
    def test_generate_cmake_file_msg_srv(self):
        cmake_lines = generate_cmake_file(
            ...
            version="0.0.1",
            scripts=[],
            package_dir={"foo.msg": "msg", "foo.srv": "srv", "": "src"},
    ...
    def test_generate_cmake_file_invalid(self):
        self.assertRaises(
            ...
            version="0.0.1",
            scripts=[],
            package_dir={"foo.sub1": "sub1", "foo.sub2": "somewhere", "": "src"},
    ...
    def test_interrogate_setup_py(self):
        try:
            ...
            with open(outfile, "r") as fhand:
                contents = fhand.read()
            self.assertEqual(
                """set(foo_SETUP_PY_SETUP_MODULE "setuptools")
