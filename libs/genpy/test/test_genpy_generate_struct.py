...
def test_pack():
    ...
    assert "buff.write(_get_struct_3lL3bB().pack(foo, bar))" == pack(
        "lllLbbbB", "foo, bar"
    )
...
def test_pack2():
    ...
    assert "buff.write(struct.Struct(patt_name).pack(foo, bar))" == pack2(
        "patt_name", "foo, bar"
    )
...
def test_unpack():
    ...
    assert "var_x = _get_struct_I3if2I().unpack(bname)" == unpack(
        "var_x", "IiiifII", "bname"
    )
...
def test_compute_struct_pattern():
    ...
    assert "bBhHiIqQfd" == compute_struct_pattern(
        [
            "int8",
            "uint8",
            "int16",
