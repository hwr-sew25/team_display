...
def make_bash_pre_command(strings, currentword):
    return "bash -c '. %s; export COMP_WORDS=(%s); export COMP_CWORD=%s;" % (
        os.path.join(PKG_PATH, "rosbash"),
        " ".join(['"%s"' % w for w in strings]),
        currentword,
    )
...
class TestWithFiles(unittest.TestCase):
    ...
    def test_make_precommand(self):
        self.assertEqual(
            'bash -c \'. %s; export COMP_WORDS=("foo" "bar"); export COMP_CWORD=1;'
            % os.path.join(PKG_PATH, "rosbash"),
            make_bash_pre_command(["foo", "bar"], 1),
        )
        self.assertEqual(
            'bash -c \'. %s; export COMP_WORDS=("foo"); export COMP_CWORD=2;'
            % os.path.join(PKG_PATH, "rosbash"),
            make_bash_pre_command(["foo"], 2),
        )
    ...
    def test_roslaunch_completion(self):
        ...
        p = subprocess.Popen(
            cmd, shell=True, stdout=subprocess.PIPE, cwd=self.test_root_path
        )
